...
function rmDir(dirPath){
	...
	function cycleSlides(){
		...
		var req = http.request(options, function(res) {
			...
			res.on('end',function(){
				...
				// Obtener SplashScreen
				getContentINIT('mediaserver.intelimedia.cl', '/ajax/infoPantalla.jsp?id='+id+'&id_categoria='+id_categoria+'&id_instancia=1');
	...
	function processContent(info){
		id = info.id;
		id_categoria = info.id_categoria;
		var tiempo = info.tiempo;
		var esVideo = info.esVideo;
		...
		if(vidArray.length!=0){
			for(key in vidArray){
				...
				// newhtmlContent = '<video autoplay id="video" src="'+vidPath+'" style="position: fixed; top: 50%; left: 50%; min-width: 100%; min-height: 100%; width: auto; height: auto; z-index: 9999; -webkit-transform: translateX(-50%) translateY(-50%); transform: translateX(-50%) translateY(-50%); background-size: cover;"></video>';
				newhtmlContent = vidPath;
			}
		}
		
		//Add to Global Array with all Slides content
		slidesContentArray.push({"esVideo" : esVideo, "tiempo" : tiempo, "contenido" : newhtmlContent});
		...
		if(slidesAcum <= slidesCount){
			...
			//Writing to ./slides/slides.json in order to player.js to read it and "PLAY!!!"
			var resultObj = new Object();
			resultObj = slidesContentArray;
			var resultStr = JSON.stringify(resultObj, null, 2);
			fs.writeFileSync('./slides/slides.json', resultStr);
		}
	}
		...
		// ==== Funciones para obtencion y procesamiento de SplashScreen ====



		function getContentINIT(_hostname, _path){
		...
		req.on('error', function(e) {
			...
			req.end();
			setTimeout(function(){
				getContentINIT(_hostname, _path);
			},1500)
		});
	...
	function processContentINIT(info){
		var tiempo = info.tiempo;
		var esVideo = info.esVideo;
		...
		//Add to Global Array with all Slides content
		slidesContentArray.push({"esSplashScreen" : "1", "slidesCount" : slidesCount, "esVideo" : esVideo, "tiempo" : tiempo, "contenido" : newhtmlContent});
	}